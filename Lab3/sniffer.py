...
# This function reads the Ethernet header (first 14 bytes).
def ethernet_dissect(ethernet_data):
    dest_mac, src_mac, l2_protocol = struct.unpack('!6s6sH', ethernet_data[:14])  # Unpack destination MAC, source MAC, and protocol.
    return {
        # Convert MAC address into readable format.
        ...
        "Data": ethernet_data[14:],  # Return rest of packet after Ethernet header.
    }

# This function reads the IPv4 header
def ipv4_dissect(ip_data):
 version_ihl, tos, total_length, identification, flags_offset, ttl, l3_protocol, checksum, src, dest = \
     struct.unpack('!BBHHHBBH4s4s', ip_data[:20])  # Unpack first 20 bytes of IP header.

 version = version_ihl >> 4 # First 4 bits = version
 ihl = version_ihl & 0x0F # Last 4 bits = header length
 header_len = ihl * 4
 ...
 return {
     ...
     'Data': ip_data[header_len:] # Return data after IP header
 }


# This function reads the TCP header.
...
# This function reads the UDP header.
...
# This function reads the ICMP header.
...
# Parse only the 12-byte DNS header.
def parse_dns(data):
...
def main():
    ETH_P_ALL = 0x0003
    packets = socket.socket(socket.PF_PACKET, socket.SOCK_RAW, socket.htons(ETH_P_ALL)) # Create raw socket to capture all packets.

    while True:
        raw_data, address = packets.recvfrom(65536)
        #print(raw_data)

        eth_info = ethernet_dissect(raw_data) #Ethernet
        #print(f"Destination MAC: {eth_info['Destination MAC']}, Source MAC: {eth_info['Source MAC']}, L2_Protocol: {eth_info['L2_Protocol']}")
        if eth_info['L2_Protocol'] == 0x0800:  # IPv4
            ip_info = ipv4_dissect(eth_info['Data'])
    
            #print(f"Source IP: {ip_info['Source IP']}, "f"Destination IP: {ip_info['Destination IP']}, "f"L3 Protocol: {ip_info['L3_Protocol']}")
            ...
            if ip_info['L3_Protocol'] == 17:  # UDP
                ...
                if udp_info:
                    print(f"UDP {udp_info['Source Port']} -> {udp_info['Destination Port']}")
            ...
            if ip_info['L3_Protocol'] == 1:  # ICMP
                ...
                if icmp_info:
                    ...
                    f"Code: {icmp_info['Code']}, "
                    f"Checksum: {icmp_info['Checksum']}")
